...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const apiUrl =
      'https://open.jpbeta.com/stockapi/searchFund/' +
      customerCd +
      '?&limit=25&start=0';
    fetch(apiUrl, {
      method: 'get',
      ...
      .then(responseJson => {
        ...
        for (let key in fundDataList) {
          ...
          Object.keys(itemList).forEach(i => {
            if (Number(i) === Number(currentIdx)) {
              totalAmountArr.push(
                parseFloat(strToInt(itemList[i].currentPrice))
              );
        ...
        const fundData = {
          ...
          // 運用利益パーセント
          percentageAmount: round(
            ((totalAmountArr.reduce(reducer) - itemPriceArr.reduce(reducer)) /
              itemPriceArr.reduce(reducer)) *
              100,
            1000
          ),
          // 運用利益
  ...
  return (
    <Page className={classes.root} title="ダッシュボード">
      <Container maxWidth={false}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={4} lg={3}>
            <Box textAlign="right">{fundData.currentDate[0]}時点</Box>
          </Grid>
          ...
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <Profit
              profitAmount={fundData.profitAmount}
              percentageAmount={fundData.percentageAmount}
            />
          </Grid>
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <Ratio
              ratioDiv={fundData.ratioDiv}
              ratioAmount={fundData.ratioAmount}
            />
          </Grid>
          ...
          {Object.keys(fundData.fundDataList).map((title, index) => (
            <Grid item lg={12} md={12} xl={12} xs={12} key={index}>
              <FundTables
                title={title}
                dataList={fundData.fundDataList[title]}
              />
            </Grid>
          ))}
        </Grid>
